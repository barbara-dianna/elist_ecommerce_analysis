...
--What products brought in the most revenue?
--Which countries and states within the top country have brought in the most revenue and how much?
--What time of day brought in the most revenue?


--Check to see that all different product types cost the same
...
--Which product brought in the most revenue 
  --Obtaining aggregate revenue grouped by product, sort by this column in descending order
  --Obtaining the quantity of each item sold using a COUNT(*) function, grouped by product
  --Displaying price of each item using AVG since all items within each prcategory have the same price
SELECT 
    product,
    SUM(price) AS revenue_by_product,
    COUNT(*) AS product_quantity_sold,
...
--How many transactions were there per country, in order from greatest to least?
SELECT 
    country,
    COUNT(*) AS transactions_per_country
FROM sales
GROUP BY country
HAVING COUNT(*) > 5
ORDER BY transactions_per_country DESC
;

--Which countries contributed the most revenue?
SELECT 
    country,
    SUM(price) AS country_revenue
FROM sales
GROUP BY country
HAVING country_revenue > 3600
ORDER BY country_revenue DESC
;

--Which states in the U.S. contributed the most revenue?
SELECT 
    state,
    SUM(price) AS US_state_revenue
FROM sales
GROUP BY state, country
HAVING US_state_revenue > 3600 AND country = 'United states'
ORDER BY US_state_revenue DESC
;

--Were the most sales made in the morning, the afternoon, or the evening?
--1) First, we need to split the transaction date column into a date column and a time column by splitting where there's a space ' '. 
SELECT 
    SUBSTR(transaction_date, 1, INSTR(transaction_date, ' ') -1) AS date,
    SUBSTR(transaction_date, INSTR(transaction_date, ' ') +1) AS time  
FROM sales
LIMIT 3
;

--2) We need to categorize times into morning, afternoon, and evening using CASE
--Can't use time as a column name since it's not in the sales database, so we'll need to replace column name with some of the code in line 182
SELECT
COUNT(*) AS number_of_sales,
    CASE
        WHEN SUBSTR(SUBSTR(transaction_date, INSTR(transaction_date, ' ') +1), '%h:%i %p') < '12:00:00' THEN 'Morning'
        WHEN SUBSTR(SUBSTR(transaction_date, INSTR(transaction_date, ' ') +1), '%h:%i %p') < '17:00:00' THEN 'Afternoon'
        ELSE 'Evening'
    END AS time_of_day
FROM sales
